...
def create_file_list(folder):
    ...
    for file in file_list:
        with open(folder + "/" + file, encoding='utf-8') as _temp_file:  # Поочередно считываем файлы
...
def create_merget_file(folder, filename):
    with open(filename + '.txt', 'w+', encoding='utf-8') as merget_file:  # Откроем (создадим) итоговый файл с именем "filename".txt
